function SelectorGadget() {
  ...
  this.sg_div = null;
	this.ignore_class = 's_42sg_g_ignore';
  this.unbound = false;
...
SelectorGadget.prototype.makeBorders = function(orig_elem, makeRed) {
  ...
  if (makeRed || elem.hasClass("s_42sg_g_suggested") || elem.hasClass("s_42sg_g_selected")) {
    this.b_top.addClass('s_42sg_g_border_red');
    this.b_bottom.addClass('s_42sg_g_border_red');
    this.b_left.addClass('s_42sg_g_border_red');
    this.b_right.addClass('s_42sg_g_border_red');
  } else {
    if (this.b_top.hasClass('s_42sg_g_border_red')) {
      this.b_top.removeClass('s_42sg_g_border_red');
      this.b_bottom.removeClass('s_42sg_g_border_red');
      this.b_left.removeClass('s_42sg_g_border_red');
      this.b_right.removeClass('s_42sg_g_border_red');
    }
  }
...
SelectorGadget.prototype.setupBorders = function() {
  if (!this.b_top) {
    ...
    this.b_top = jQuerySG('<div>').addClass('s_42sg_g_border').css('height', width).hide().bind("mousedown.sg", { 'self': this }, this.sgMousedown);
    this.b_bottom = jQuerySG('<div>').addClass('s_42sg_g_border').addClass('s_42sg_g_bottom_border').css('height', this.px(this.border_width + 6)).hide().bind("mousedown.sg", { 'self': this }, this.sgMousedown);
    this.b_left = jQuerySG('<div>').addClass('s_42sg_g_border').css('width', width).hide().bind("mousedown.sg", { 'self': this }, this.sgMousedown);
    this.b_right = jQuerySG('<div>').addClass('s_42sg_g_border').css('width', width).hide().bind("mousedown.sg", { 'self': this }, this.sgMousedown);
...
SelectorGadget.prototype.sgMouseover = function(e) {
  ...
  if (gadget.special_mode != 'd') { // Jump to any the first selected parent of this node.
    ...
    if (!jQuerySG('.s_42sg_g_selected', this).get(0)) {
			if (gadget.selectable(self)) gadget.makeBorders(this);
    }
  }
  return false;
...
SelectorGadget.prototype.firstSelectedOrSuggestedParent = function(elem) {
  var orig = elem;
  if (jQuerySG(elem).hasClass('s_42sg_g_suggested') || jQuerySG(elem).hasClass('s_42sg_g_selected')) return elem
...
SelectorGadget.prototype.highlightIframe = function(elem, click) {
  ...
  var block = jQuerySG('<div>').css('position', 'absolute').css('z-index', '99998').css('width', this.px(elem.outerWidth())).
                              ...
                              css('background-color', '#AAA').css('opacity', '0.6').addClass("s_42sg_g_iframe").addClass('s_42sg_g_clean');
  ...
  var instructions = jQuerySG("<div><span>This is an iframe.  To select in it, </span></div>").
															css({ width: "200px", border: "1px solid #888", 
																		...
																		height: "150px" }).
															addClass("s_42sg_g_iframe_info").addClass("s_42sg_g_iframe").addClass('s_42sg_g_clean');
...
SelectorGadget.prototype.unhighlightIframes = function(elem, click) {
	jQuerySG(".s_42sg_g_iframe").remove();
};
...
SelectorGadget.prototype.sgMousedown = function(e) {
  ...
  if (w_elem.hasClass('s_42sg_g_border')) {
  ...
  if (gadget.special_mode != 'd') {
    ...
    if (jQuerySG('.s_42sg_g_selected', this).get(0)) gadget.blockClicksOn(elem); // Don't allow selection of elements that have a selected child.
  }
  ...
  if (w_elem.hasClass('s_42sg_g_selected')) {
    w_elem.removeClass('s_42sg_g_selected');
    gadget.selected.splice(jQuerySG.inArray(elem, gadget.selected), 1);
  } else if (w_elem.hasClass("s_42sg_g_rejected")) {
    w_elem.removeClass('s_42sg_g_rejected');
    gadget.rejected.splice(jQuerySG.inArray(elem, gadget.rejected), 1);
  } else if (w_elem.hasClass("s_42sg_g_suggested")) {
    w_elem.addClass('s_42sg_g_rejected');
    gadget.rejected.push(elem);
  } else {
    w_elem.addClass('s_42sg_g_selected');
...
SelectorGadget.prototype.setupEventHandlers = function() {
  jQuerySG("*:not(.s_42sg_g_ignore)").bind("mouseover.sg", { 'self': this }, this.sgMouseover);
  jQuerySG("*:not(.s_42sg_g_ignore)").bind("mouseout.sg", { 'self': this }, this.sgMouseout);
  jQuerySG("*:not(.s_42sg_g_ignore)").bind("mousedown.sg", { 'self': this }, this.sgMousedown);
...
SelectorGadget.prototype.suggestPredicted = function(prediction) {
  if (prediction && prediction != '') {
    ...
    jQuerySG(prediction).each(function() {
      count += 1;
      if (!jQuerySG(this).hasClass('s_42sg_g_selected') && !jQuerySG(this).hasClass('s_42sg_g_ignore') && !jQuerySG(this).hasClass('s_42sg_g_rejected')) jQuerySG(this).addClass('s_42sg_g_suggested');
    });
...
SelectorGadget.prototype.clearSelected = function(e) {
  ...
  self.selected = [];
  self.rejected = [];
  jQuerySG('.s_42sg_g_selected').removeClass('s_42sg_g_selected');
  jQuerySG('.s_42sg_g_rejected').removeClass('s_42sg_g_rejected');
  self.removeBorders();
  self.clearSuggested();
};
...
SelectorGadget.prototype.clearSuggested = function() {
  jQuerySG('.s_42sg_g_suggested').removeClass('s_42sg_g_suggested');
...
SelectorGadget.prototype.makeInterface = function() {
  this.sg_div = jQuerySG('<div>').attr('id', '_s_42_g_div').addClass('s_42sg_g_bottom').addClass('s_42sg_g_ignore');
...
SelectorGadget.prototype.makeStandardInterface = function() {
  var self = this;
  var path = jQuerySG('<input>').attr('id', '_sg_path_field').addClass('s_42sg_g_ignore').addClass('s_42sg_g_input_field').keydown(function(e) {
    if (e.keyCode == 13) {
      return self.refreshFromPath(e);
    }
  }).focus(function() { jQuerySG(this).select(); });
  this.sg_div.append(path);
  this.clear_button = jQuerySG('<input type="button" value="Clear"/>').bind("click", {'self': this}, this.clearEverything).addClass('s_42sg_g_ignore').addClass('s_42sg_g_input_field');
  this.sg_div.append(this.clear_button);
  this.sg_div.append(jQuerySG('<input type="button" value="Toggle Position"/>').click(function() {
    if (self.sg_div.hasClass('s_42sg_g_top')) {
      self.sg_div.removeClass('s_42sg_g_top').addClass('s_42sg_g_bottom');
    } else {
      self.sg_div.removeClass('s_42sg_g_bottom').addClass('s_42sg_g_top');
    }
  }).addClass('s_42sg_g_ignore').addClass('s_42sg_g_input_field'));

  this.sg_div.append(jQuerySG('<input type="button" value="XPath"/>').bind("click", {'self': this}, this.showXPath).addClass('s_42sg_g_ignore').addClass('s_42sg_g_input_field'));

  this.sg_div.append(jQuerySG('<input type="button" value="Help"/>').bind("click", {'self': this}, this.showHelp).addClass('s_42sg_g_ignore').addClass('s_42sg_g_input_field'));

  this.sg_div.append(jQuerySG('<input type="button" value="X"/>').bind("click", {'self': this}, this.unbindAndRemoveInterface).addClass('s_42sg_g_ignore').addClass('s_42sg_g_input_field'));

  // this.sg_div.append( 
  //   jQuerySG('<span class="s_42sg_g_ignore s_42sg_g_option s_42sg_g_new_line">Show XPath</span>').bind("click", {'self': this}, this.showXPath)
  // );
