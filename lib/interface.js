...
function setupEventHandlers() {
  $("*:not(.sg_ignore)").bind("mouseover", sg_mouseover);
  $("*:not(.sg_ignore)").bind("mouseout", sg_mouseout);
  $("*:not(.sg_ignore)").bind("click", sg_click);
  $("html").bind("keydown", listen_for_action_keys);
  $("html").bind("keyup", clear_action_keys);
}

function removeEventHandlers() {
  $("*:not(.sg_ignore)").unbind("mouseover", sg_mouseover);
  $("*:not(.sg_ignore)").unbind("mouseout", sg_mouseout);
  $("*:not(.sg_ignore)").unbind("click", sg_click);
...
function makeInterface() {
  var sg_div = $('<div>').attr('id', '_sg_div').addClass('sg_right').addClass('sg_ignore');
  ...
  sg_div.append(path);
  sg_div.append($('<input type="button" value="Go"/>').click(refreshFromPath).attr('class', 'sg_ignore'));
  sg_div.append($('<input type="button" value="Clear"/>').click(clearSelected).attr('class', 'sg_ignore'));
  sg_div.append($('<input type="button" value="Toggle Position"/>').click(function() {
    if (sg_div.hasClass('sg_right')) {
      sg_div.removeClass('sg_right').addClass('sg_left');
    } else {
      sg_div.removeClass('sg_left').addClass('sg_right');
    }
