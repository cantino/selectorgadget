...
SelectorGadget.prototype.makeBorders = function(elem, makeRed) {
  ...
  var path_to_show = elem.tagName.toLowerCase();
	...
	if(jQuery(elem).attr("implied"))
	 path_to_show = jQuery(elem).attr("implied");
...
SelectorGadget.prototype.sg_click = function(e) {
  ...
  if (gadget.special_mode == 'd') {
    ...
    if (jQuery('.sg_selected', this).get(0)) return; // Don't allow selection of elements that have a selected child.
  }
...
SelectorGadget.prototype.setupEventHandlers = function() {
  ...
  jQuery("*:not(.sg_ignore)").bind("mousedown.sg", { 'self': this }, this.sg_click);
...
SelectorGadget.prototype.blockClicksOn = function(elem) {
  ...
  setTimeout(function() { block.remove(); },250)  
};
...
SelectorGadget.prototype.save = function() {
	var sg_div = jQuery('#_sg_div');
	var hiddenField = function(label, value){
		var elem = document.createElement("INPUT");
		return jQuery(elem).attr("type", "hidden").attr("name", label).attr("value", value);
	};
	var id = "_sg_save" + (new Date()).getTime();
	var endpoint = selectorGadgetIsLocal ? "http://localhost:3000/api/post" : "http://selectropolis.kylemaxwell.com/api/post";
	var selector = jQuery('#_sg_path_field').val();
	if('No valid path found.' == selector) {
		alert("Please select something");
		return;
	}
	var label = prompt("Label for this field?");
	if(!label) return;
	var url = location.href;
	var content = document.body.innerHTML;
	var iframe = jQuery("<iframe>").attr("name", id).attr("id", "ifr"+id).attr("width", 1).attr("height", 1).attr("style", "position:absolute;left:-100");
	var form = jQuery("<form>").attr("target", id).attr("method", "post").attr("action", endpoint).attr("id", "frm"+id);
	form.append(hiddenField("label", label));
	form.append(hiddenField("selector", selector));
	form.append(hiddenField("url", url));
	form.append(hiddenField("content", content));
	sg_div.append(iframe)
	sg_div.append(form)
	var opt = document.createElement("OPTION");
	opt.text = label + " -- " + selector;
	opt.value = selector;
	jQuery('#_sg_selectors').append(opt);
	
	form.submit();
	var self = this;
	setTimeout("jQuery('#ifr"+id+"').remove()", 30000);
	setTimeout("jQuery('#frm"+id+"').remove()", 500);
};

SelectorGadget.prototype.cleanupSave = function(id) {
	var sg_div = jQuery("#_sg_div");
	jQuery("#ifr"+id).remove();
	jQuery("#frm"+id).remove();
}
...
SelectorGadget.prototype.makeInterface = function() {
  ...
  var path = jQuery('<input>').attr('id', '_sg_path_field').attr('class', 'sg_ignore').keydown(function(e) {
	...
	var selectors = jQuery(document.createElement("SELECT")).attr("size", 5).attr('class', 'sg_ignore').attr('id', '_sg_selectors');
	var pages = jQuery(document.createElement("SELECT")).attr("size", 5).attr('class', 'sg_ignore').attr('id', '_sg_pages');
	this.sg_div.append(selectors);
	this.sg_div.append(pages);
	this.sg_div.append(document.createElement("BR"));
  this.sg_div.append(path);
	this.reloadSelectors(selectors, pages);
  this.sg_div.append(jQuery('<input type="button" value="Save"/>').bind("click",  {'self': this}, this.save).attr('class', 'sg_ignore'));
...
SelectorGadget.prototype.reloadSelectors = function(selectors, pages) {
	var self = this;
	jQuery.ajax({
		url: (selectorGadgetIsLocal ? "http://localhost:3000/api/list" : "http://selectropolis.kylemaxwell.com/api/list") + ("?url="+location.href),
		cache: false,
		dataType: "jsonp",
		success: function(obj, textStatus){
			var data = obj["selectors"];
			self.loadSelectors(selectors, data);
			...
			for(var i = 0; i < data.length; i++) {
				...
				opt.text = data[i][1];
				pages.append(opt);
				jQuery(opt).attr('class', 'sg_ignore');
				pages.click(function(){
					self.loadSelectors(selectors, data[this.value][2]);
				});
			}
		}
	});
...
SelectorGadget.prototype.loadSelectors = function(selectors, data) {
	var self = this;
	selectors.empty();
	jQuery(".sg_implied").removeClass("sg_implied");
	for(var i = 0; i < 8; i++) {
		var c = "sg_c" + i;
		jQuery("."+c).removeClass(c);
	}
	for(var i = 0; i < data.length; i++) {
		var c = i > 7 ? "sg_c7" : "sg_c" + i;
		var opt = document.createElement("OPTION");
		opt.value = data[i][0];
		opt.text = data[i][1];
		selectors.append(opt);
		jQuery(opt).attr('class', 'sg_ignore');
		selectors.click(function(){
			jQuery('#_sg_path_field').val(this.value);
			self.refreshFromPath(false);
		});
		jQuery(opt).addClass(c);
		jQuery(opt.value).addClass("sg_implied").attr("implied", opt.text).addClass(c);
	}
}
