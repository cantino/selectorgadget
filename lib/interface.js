...
SelectorGadget.prototype.suggestPredicted = function(prediction) {
  if (prediction && prediction != '') {
    ...
    if (this.clear_button) {
      var suggested = jQuery('.sg_suggested');
      if (suggested.get(0)) {
        this.clear_button.attr('value', 'Clear (' + (suggested.length + 1) + ')');
      } else {
...
SelectorGadget.prototype.makeInterface = function() {
  ...
  this.clear_button = jQuery('<input type="button" value="Clear"/>').bind("click", {'self': this}, this.clearSelected).attr('class', 'sg_ignore');
