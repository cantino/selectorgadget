...
SelectorGadget.prototype.makeInterface = function() {
  ...
  this.sg_div.append(path);
	this.reloadSelectors(selectors);
...
SelectorGadget.prototype.reloadSelectors = function(selectors) {
	var self = this;
	jQuery.ajax({
		...
		success: function(obj, textStatus){
			...
			for(var i = 0; i < data.length; i++) {
				...
				opt.value = data[i][0];
				opt.text = data[i][1];
				jQuery(opt).attr('class', 'sg_ignore').click(function(){
					jQuery('#_sg_path_field').val(this.value);
					self.refreshFromPath();
				});
