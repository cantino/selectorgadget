// Wizard methods for SelectorGadget
...
SelectorGadget.prototype.makePathField = function() {
  var self = this;
  var path = jQuerySG('<input>').attr('id', '_sg_path_field').keydown(function(e) {
    if (e.keyCode == 13) {
...
SelectorGadget.prototype.makeClearButton = function() {
  this.clear_button = jQuerySG('<input type="button" value="Clear"/>').addClass('s_42sg_g_input_field').bind("click", {'self': this}, this.clearEverything);
  return this.clear_button;
};

SelectorGadget.prototype.makeRemoteSaveButton = function(field_name) {
  var self = this;
  return jQuerySG('<input type="button" value="Save"/>').addClass('s_42sg_g_input_field').click(function() {
...
SelectorGadget.prototype.makeCloseButton = function(text) {
  return jQuerySG('<input type="button" />').addClass('s_42sg_g_input_field').attr("value", text).bind("click", {'self': this}, this.unbindAndRemoveInterface);
};

SelectorGadget.prototype.makeNextButton = function(text, card) {
  var self = this;
  return jQuerySG('<input type="button" />').addClass('s_42sg_g_input_field').attr("value", text).click(function() {
...
SelectorGadget.prototype.displayWizardCard = function(card) {
  this.sg_div.html("");
  var our_div = jQuerySG("<div></div>").addClass("s_42sg_g_wizard");
  this.clearEverything();
  this.unbind();
  this.css_restriction = card.restrict_css_to;
  
  our_div.append(jQuerySG("<div></div>").addClass("s_42sg_g_prompt").html(card.prompt));
