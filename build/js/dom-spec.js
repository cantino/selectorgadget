(function() {
  ...
  describe("DomPredictionHelper", function() {
    ...
    return describe("cssToXPath", function() {
      beforeEach(function() {
        var fixture_contents, fixture_name, results;
        results = [];
        for (fixture_name in fixtures) {
          fixture_contents = fixtures[fixture_name];
          results.push(jQuerySG("#jasmine-content").append(fixture_contents));
        }
        return results;
      });
      return it("converts css to xpath", function() {
        var css, cssAndXPathMatch, expressions, i, len, results;
        cssAndXPathMatch = function(css, xpath) {
          ...
          while (elem = elems.iterateNext()) {
            if (indexOf.call(css_matches, elem) < 0) {
              return false;
            }
          }
          return true;
        ...
        for (i = 0, len = expressions.length; i < len; i++) {
          css = expressions[i];
          results.push(expect(cssAndXPathMatch(css, dom.cssToXPath(css))).toBeTruthy());
        }
        return results;
      });
    });
